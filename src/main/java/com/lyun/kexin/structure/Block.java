    ...
    public static void analysisBlock(StringBuilder res, BlockStmt blockStmt){
        if (blockStmt.getChildNodes().size()>0){
            for (Node childNode : blockStmt.getChildNodes()) {
                if (childNode instanceof ExpressionStmt){
                    //
                    res.append(analysisExpressionStmt((ExpressionStmt) childNode));
                }else if (childNode instanceof ReturnStmt){
                    //返回块
                    ReturnStmt returnStmt = (ReturnStmt) childNode;
                    res.append("return ");
                    ...
                    if (((IfStmt) childNode).hasElseBlock()){
                        res.append("move next\n");
...
//                    res.append(analysisBlock();)
                ...
                }else if (childNode instanceof BreakStmt){
                    res.append("break\n");
